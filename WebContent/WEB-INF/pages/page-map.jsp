...
<head>
	...
	$(function() {
		...
		$("div#menu-detail>div>a>small:contains('寵物友善餐廳')").parent().addClass("active");
		// 分隔線要顯示
       	...
       	function createMarker(data){
            // 當有多筆資料的時候跑迴圈, 把每一個定義好的maker都加到集合體上.
       		for (let i = 0; i < data.length; i++) {
                markers.addLayer(L.marker([data[i].lat, data[i].lng],{icon:myIcon}).bindPopup(
                    "<p class='toolong'><b style='font-size: medium;'>"+data[i].storeName+"</b><p>地址: "+data[i].address+"<p>電話: "+data[i].phone
                    +"<p><img src='assets/img" + data[i].mapImage + "' width='180px' height='120px'>").bindTooltip(data[i].storeName,{direction: 'left',
                                        sticky: true,
                                        permanent: false, 
                                        opacity: 1.0
